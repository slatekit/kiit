...
| **date**| 2017-04-12T22:59:15.522 |
| **version** | 1.4.0  |
| **jar** | slate.core.jar  |
| **namespace** | slate.core.apis  |
| **source core** | slate.core.apis.ApiContainer.scala  |
| **source folder** | [/src/lib/scala/Slate.Core/src/main/scala/slate/core/apis](https://github.com/code-helix/slatekit/tree/master/src/lib/scala/Slate.Core/src/main/scala/slate/core/apis)  |
| **example** | [/src/apps/scala/slate-examples/src/main/scala/slate/examples/Example_Api.scala](https://github.com/code-helix/slatekit/tree/master/src/apps/scala/slate-examples/src/main/scala/slate/examples/Example_Api.scala) |
...
```scala
  ...
  // =======================================================================================
  @Api(area = "app", name = "users", desc = "registers users", roles= "?", auth = "app-roles", verb = "*", protocol = "*")
  class AppUsersApi(ctx:AppContext) extends ApiBase(ctx) {
    var user = new User
    ...
    @ApiAction(name = "", desc = "get total number of users", roles = "@parent")
    def rawCommand(cmd:Request) : Boolean = {
  ...
  // =======================================================================================
  @Api(area = "app", name = "about", desc = "registers users", roles= "admin", auth = "key-roles", verb="*", protocol = "*")
  class AppVersionApi(ctx:AppContext) extends ApiBase(ctx) {
    var user = new User
  ...
  // =======================================================================================
  class MyAuthProvier(keys:Option[List[ApiKey]]) extends Auth(keys, None)
    with ResultSupportIn
  {
    ...
    /**
      ...
      * @return
      */
    override def isKeyRoleValid(cmd:Request, roles:String, rolesParent:String):Result[Boolean] = {
    ...
    /**
      ...
      * @return
      */
    override def isAppRoleValid(cmd:Request, roles:String, roleParent:String):Result[Boolean] = {
      ...
      val header = cmd.opts.fold("user_01_sample_token")( headers => {
  ...
  object Setup {
    ...
    val auth = Some(new MyAuthProvier(Some(
      List[ApiKey](
        ApiKey("devs" , "94744D43418C421586E544DE0BB4CC37", "dev"          ),
        ApiKey("ops"  , "677961A57F82440AB6F4E1F1EAC721ED", "dev,ops"      ),
        ApiKey("admin", "A100E22B5544423693FABD81791FAC80", "dev,ops,admin")
      ))))
...
```scala
    ...
    apis.register[AppUsersApi](new AppUsersApi(ctx))

    // REG 2. Register the api ( allow API actions from subclass also )
    apis.register[AppUsersApi](new AppUsersApi(ctx), declaredOnly = false )
    ...
    apis.register[AppUsersApi](new AppUsersApi(ctx), declaredOnly = false, roles = Some("admin"))
    ...
    apis.register[AppUsersApi](new AppUsersApi(ctx), declaredOnly = false, roles = Some("admin"), protocol = Some("cli"))

    // REG 4b. Register the api and set access to api only from web/http container
    //        This overrides the "access" value in the @Api annotation
    apis.register[AppUsersApi](new AppUsersApi(ctx), declaredOnly = false, roles = Some("admin"), protocol = Some("web"))

    // REG 4c. Register the api and set access to api from both CLI/Web
    //        This overrides the "access" value in the @Api annotation
    apis.register[AppUsersApi](new AppUsersApi(ctx), declaredOnly = false, roles = Some("admin"), protocol = Some("*"))
    ...
    apis.register[AppVersionApi](new AppVersionApi(ctx), declaredOnly = false, roles = Some("admin"), protocol = Some("*"), auth = Some("key-roles"))
