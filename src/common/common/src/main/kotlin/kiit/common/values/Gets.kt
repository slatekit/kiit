...
interface Gets {
    ...
    fun getDateTimeOrElse(key: String, default: DateTime): DateTime =
        getOrElse(key, { k: String -> getDateTime(k) }, default)
    ...
    fun getZonedDateTimeOrElse(key: String, default: ZonedDateTime): ZonedDateTime =
        getOrElse(key, { k: String -> getZonedDateTime(k) }, default)

    fun getZonedDateTimeUtcOrElse(key: String, default: ZonedDateTime): ZonedDateTime =
        getOrElse(key, { k: String -> getZonedDateTimeUtc(k) }, default)

    fun getUUIDOrElse(key: String, default: UUID): UUID =
        getOrElse(key, { k: String -> UUID.fromString(getString(k)) }, default)

    fun getUPIDOrElse(key: String, default: UPID): UPID =
        getOrElse(key, { k: String -> UPIDs.parse(getString(k)) }, default)
