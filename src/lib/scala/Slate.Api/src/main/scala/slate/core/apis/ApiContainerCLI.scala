...
class ApiContainerCLI(ctx    : AppContext                    ,
  ...
  {
    // Guard
    require(_lookup.contains(area), s"${area} not found")
  ...
  override def handleHelpForApi(area:String, apiName:String):Unit =
  {
    // Guard
    require(_lookup.contains(area), s"${area} not found")
    require( _lookup(area).contains(apiName), s"${apiName} not found")
  ...
  override def handleHelpForAction(area:String, apiName:String, actionName:String):Unit =
  {
    ...
    require(_lookup.contains(area), s"${area} not found")
    require( _lookup(area).contains(apiName), s"${apiName} not found")
    require( _lookup(area)(apiName).contains(actionName), s"${actionName} not found")
