...
class DateTime(private val _date: LocalDateTime )  {
  ...
  /**
    ...
    * @param d
    */
  def this(d:ZonedDateTime) =
  {
    this(d.toLocalDateTime)
  }


  def this(d:Date) =
  {
    this(DateTime.create(d))
  }
  ...
  {
    new DateTime(_date.plusYears(years))
  }


  def addMonths(months: Int) : DateTime =
  {
    new DateTime(_date.plusMonths(months))
  }


  def addDays(days: Int) : DateTime =
  {
    new DateTime(_date.plusDays(days))
  }


  def addHours(hours: Int) : DateTime =
  {
    new DateTime(_date.plusHours(hours))
  }


  def addMins(mins: Int) : DateTime =
  {
    new DateTime(_date.plusMinutes(mins))
  }


  def addMinutes(mins: Int) : DateTime =
  {
    new DateTime(_date.plusMinutes(mins))
  }


  def addSeconds(secs: Int) : DateTime =
  {
    new DateTime(_date.plusSeconds(secs))
  }


  def timeOfDay() : TimeSpan =
  {
  ...
  def atUtc() : DateTime =
  {
    new DateTime(_date.atZone(ZoneId.of(DateTime.UTC) ))
  }


  def atZone(zone: String) : DateTime =
  {
    new DateTime(_date.atZone(ZoneId.of(zone)))
  }
  ...
  def toStringYYYYMMDD() : String = {
    ...
    text += (if(month < 10) "0" + month else month.toString)
  ...
  def toStringYYYYMMDDHHmmss() : String = {
    ...
    text += (if(month < 10) "0" + month else month.toString)
    text += (if(day < 10 ) "0" + day else day.toString)
    text += (if(hours   < 10 ) "0" + hours else hours )
    text += (if(minutes < 10 ) "0" + minutes else minutes)
...
object DateTime {
  ...
  def min(): DateTime =
  {
    new DateTime(LocalDateTime.MIN)
  }
  ...
  def daysAgo(days: Int) : DateTime =
  {
    DateTime.today().addDays(-1 * days)
  }


  def daysFromNow(days: Int) : DateTime =
  {
    DateTime.today().addDays(days)
  }


  def monthsAgo(months: Int) : DateTime =
  {
    DateTime.today().addMonths(-1 * months )
  }


  def monthsFromNow(months: Int) : DateTime =
  {
  ...
  def parseNumericVal(value:String) : DateTime =
  {
    ...
    // yyyymmdd = 8 chars
      ...
      return new DateTime(Integer.parseInt(yearTxt), month - 1, Integer.parseInt(dayTxt))
    }
