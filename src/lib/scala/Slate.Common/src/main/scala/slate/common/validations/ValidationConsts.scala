...
object ValidationConsts {
  ...
  val NUMS          = "0123456789".toCharArray.map( c => c -> true).toMap
  val LETTERS_LCASE = "abcdefghijklmnopqrstuvwxyz".toCharArray.map( c => c -> true).toMap
  val LETTERS_UCASE = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".toCharArray.map( c => c -> true).toMap
  val LETTERS_ALL   = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".toCharArray.map( c => c -> true).toMap
  val ALPHA         = "0123456789abcdefghijklmnopqrstuvwxyz".toCharArray.map( c => c -> true).toMap
  val SYMS          = "!@#$%^&*()_+-=[]{}|;:,./<>?".toCharArray.map( c => c -> true).toMap
  val ALPHASYM      = "0123456789abcdefghijklmnopqrstuvwxyz!@#$%^&*()_+-=[]{}|;:,./<>?".toCharArray.map( c => c -> true).toMap
