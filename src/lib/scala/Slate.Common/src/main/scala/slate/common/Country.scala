...
/**
  ...
  */
case class Country(iso:String, iso3:String, phone:String, name:String) {
}
...
object Country {
  ...
  /**
    ...
    * @return
    */
  def usa():List[Country] = {
      List[Country](Country("US", "USA", "1", "United States"))
  }
  ...
  /**
    ...
    * @return
    */
  def filter(codes:List[CountryCode]):List[Country] = {
    val countries = all()
    val filtered = codes.flatMap( code => countries.find( c => c.iso == code.value.toUpperCase))
    filtered
  }


  /**
    * all countries and their codes, phones, names
    *
    * @return
    */
