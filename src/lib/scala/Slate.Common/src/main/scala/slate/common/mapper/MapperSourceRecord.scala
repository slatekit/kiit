...
class MapperSourceRecord(val rs:ResultSet) extends MappedSourceReader {
  ...
  override def getString(pos: Int): String = rs.getString(pos)
  override def getString(name:String): String = rs.getString(name)
  ...
  override def getFloat(pos: Int): Float = rs.getFloat(pos)
  override def getFloat(name:String): Float = rs.getFloat(name)
