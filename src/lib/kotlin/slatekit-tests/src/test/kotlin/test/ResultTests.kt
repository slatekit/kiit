...
class ResultTests {
  ...
  @Test fun can_build_result() {
    val res =  Success(123,"user01",  msg = "created", ref = 20, tag = "code12")
    assert( res.success  )
  ...
  @Test fun can_map_to_another_value() {
    val yes = yes(msg = "m1", tag = "t1", ref = "GUID_123")
  ...
  @Test fun can_map_failure() {
    val fail = no(msg = "m1", tag = "t1", ref = "GUID_123")
 ...
 @Test fun can_flat_map_to_another_value() {
    ...
    val good  = yes.flatMap( { v ->
      if(v)
        success(1, msg = "f_msg1", tag = "f_tag1", ref = "GUID_XYZ")
      else
