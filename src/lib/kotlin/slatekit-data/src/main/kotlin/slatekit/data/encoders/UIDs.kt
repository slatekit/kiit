...
open class UUIDEncoder : SqlEncoder<UUID> {
    ...
    override fun convert(value: UUID?): Value {
        return Value("", DataType.DTString, value?.toString(), encode(value))
    }
}
...
open class UPIDEncoder : SqlEncoder<UPID> {
    ...
    override fun convert(value: UPID?): Value {
        return Value("", DataType.DTString, value?.value, encode(value))
    }
}
