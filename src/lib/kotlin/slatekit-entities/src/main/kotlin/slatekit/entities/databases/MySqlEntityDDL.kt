...
class MySqlEntityDDL : EntityDDL {
    ...
    override fun buildAddTable(dbSrc: DbSource, model: Model, namer: Namer?): String
    {
        ...
        val dataFieldSql = buildColumns(dbSrc, model, true, namer)
     ...
     * Builds the table DDL sql statement using the model supplied.
     */
    fun buildColumns(dbSrc: DbSource, model: Model, filterId:Boolean, namer: Namer?): String
    {
        val buff = StringBuilder()
        ...
        val dataFieldSql = dataFields.fold("", { acc, field ->
            if(field.isEnum) {
                acc + ", " + dbSrc.buildAddCol(field.storedName, DbFieldTypeNumber, field.isRequired, field.maxLength)
            } else if(field.model != null) {
                val sql = buildColumns(dbSrc, field.model!!, false, namer)
                acc + sql
            }
            else {
