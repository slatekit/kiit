 ...
 */
data class ShellCommand(val area: String,
                        val name: String,
                        val action: String,
                        val line: String,
                        val args: Args,
    ...
    companion object ShellCommandCompanion {

        fun build(args: Args, line: String): ShellCommand {
            val area = args.getVerb(0)
            val name = args.getVerb(1)
            val action = args.getVerb(2)
            return ShellCommand(area, name, action, line, args)
        }
    }
}