...
import slatekit.functions.features.Feature
import slatekit.functions.features.Interval
import slatekit.functions.features.Limit
...
interface Events {
    ...
    fun apply(feature: Feature<WorkRequest, WorkResult>) {

    }


    fun onEvery(count:Long, op:suspend (WorkContext) -> Unit ) {
        val interval = Interval<WorkRequest, WorkResult>(count) { req, res ->
            println(req.task)
...
fun test() {
    ...
    job.worker("w2").apply(Interval(100) { req, res ->
        res
    })
