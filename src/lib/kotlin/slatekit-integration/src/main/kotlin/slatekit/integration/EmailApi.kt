...
@Api(area = "sys", name = "email", desc = "api to send emails", roles = "ops", auth = "key-roles", verb = "*", protocol = "*")
class EmailApi(val svc: EmailService, context: AppContext) : ApiWithSupport(context) {
    ...
    fun send(to: String, subject: String, body: String, html: Boolean): Result<Boolean> {
        val content = Uris.readText("user://blendlife-kotlin/templates/template_vacation.html")!!
     ...
     */
    @ApiAction(name = "", desc = "send an email", roles = "@parent", verb = "@parent", protocol = "@parent")
    fun sendFile(to: String, subject: String, filePath: String, html: Boolean): Result<Boolean> {
    ...
    fun sendUsingTemplate(name: String, to: String, subject: String, html: Boolean, vars: Vars): Result<Boolean> {
