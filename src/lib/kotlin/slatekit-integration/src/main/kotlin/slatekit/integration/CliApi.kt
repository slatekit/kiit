...
class CliApi(private val creds: Credentials,
    ...
    : CliService(ctx.dirs!!, settings, ctx.app) {
    ...
    override fun onCommandExecuteInternal(cmd: CliCommand): CliCommand {
        ...
        val apiCmd = Request.cli(cmd.line, cmd.args, opts, ApiConstants.ProtocolCLI)
     ...
     * @param mode
     */
    override fun showHelpFor(cmd: CliCommand, mode: Int): Unit {
        when (mode) {
        // 1: {area} ? = help on area
