...
class FilesApi(val files: CloudFilesBase, context: AppContext) : ApiWithSupport(context) {
    ...
    @ApiAction(desc = "creates a file with the supplied folder name, file name, and content from doc",
            roles = "@parent", verb = "@parent", protocol = "@parent")
    fun createFromDoc(folder: String, name: String, doc: Doc): Result<String> {
    ...
    @ApiAction(desc = "updates a file with the supplied folder name, file name, and content",
            roles = "@parent", verb = "@parent", protocol = "@parent")
    fun update(folder: String, name: String, content: String): Result<String> {
    ...
    @ApiAction(desc = "updates a file with the supplied folder name, file name, and content from doc",
            roles = "@parent", verb = "@parent", protocol = "@parent")
    fun updateFromDoc(folder: String, name: String, doc: Doc): Result<String> {
    ...
    @ApiAction(desc = "deletes a file with the supplied folder name, file name",
            roles = "@parent", verb = "@parent", protocol = "@parent")
    fun delete(folder: String, name: String): Result<String> {
    ...
    @ApiAction(desc = "downloads the file specified by folder and name to the local folder specified.",
            roles = "@parent", verb = "@parent", protocol = "@parent")
    fun download(folder: String, name: String, localFolder: String, display: Boolean): Result<String> {
    ...
    @ApiAction(desc = "downloads the file specified by folder and name, as text content to file supplied",
            roles = "@parent", verb = "@parent", protocol = "@parent")
    fun downloadToFile(folder: String, name: String, filePath: String, display: Boolean): Result<String> {
        return show(files.downloadToFile(folder, name, Uris.interpret(filePath) ?: filePath), display)
    }


    private fun show(result: Result<String>, display: Boolean): Result<String> {
        ...
        val output = if (display) {
            ...
            "PATH   : " + path
        return successOrError(result.success, output, result.msg, result.tag, result.ref)
    }
}
