...
class ConfigApi(context: AppContext) : ApiWithSupport(context) {
    ...
    fun createApiKey(rootDir: String, name: String, key: String, roles: String): ApiKey {
        ...
        ConfFuncs.createApiKey(rootDir, name, apiKey, context.enc)
        return apiKey
    }
    ...
    fun createDbConMySql(rootDir: String, name: String, url: String, user: String, pass: String): DbConString {
        val dbCon = DbConString(DbTypeMySql.driver, url, user, pass)
        ConfFuncs.createDbCon(rootDir, name, dbCon, context.enc)
        return dbCon
    }
