...
interface Builder<out E> {
    ...
    fun <T> errored(msg: String, status:Status? = null): Result<T, E> = Failure(errorFromStr(msg, get(status, Codes.ERRORED)), status ?: Codes.ERRORED)
    fun <T> errored(ex: Exception, status:Status? = null): Result<T, E> = Failure(errorFromEx(ex, get(status, Codes.ERRORED)), status ?: Codes.ERRORED)
    fun <T> errored(err: Err, status:Status? = null): Result<T, E> = Failure(errorFromErr(err, get(status, Codes.ERRORED)), status ?: Codes.ERRORED)
    fun <T> errored(status:Status): Result<T, E> = Failure(errorFromStr(null, get(status, Codes.ERRORED)), status )
