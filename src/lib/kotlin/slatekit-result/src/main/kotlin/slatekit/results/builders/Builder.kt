...
interface Builder<out E> {
    ...
    fun <T> denied(ex: Throwable, status: Failed.Denied? = null): Result<T, E> =
        Failure(errorFromEx(ex, Codes.DENIED), status ?: Codes.DENIED)

    fun <T> denied(err: Err, status: Failed.Denied? = null): Result<T, E> =
        Failure(errorFromErr(err, Codes.DENIED), status ?: Codes.DENIED)
    ...
    fun <T> ignored(ex: Throwable, status: Failed.Ignored? = null): Result<T, E> =
        Failure(errorFromEx(ex, Codes.IGNORED), status ?: Codes.IGNORED)

    fun <T> ignored(err: Err, status: Failed.Ignored? = null): Result<T, E> =
        Failure(errorFromErr(err, Codes.IGNORED), status ?: Codes.IGNORED)

    fun <T> ignored(status: Failed.Ignored): Result<T, E> =
        Failure(errorFromStr(null, get(status, Codes.IGNORED)), status)
    ...
    fun <T> invalid(ex: Throwable, status: Failed.Invalid? = null): Result<T, E> =
        Failure(errorFromEx(ex, Codes.INVALID), status ?: Codes.INVALID)

    fun <T> invalid(err: Err, status: Failed.Invalid? = null): Result<T, E> =
        Failure(errorFromErr(err, Codes.INVALID), status ?: Codes.INVALID)

    fun <T> invalid(status: Failed.Invalid): Result<T, E> =
        Failure(errorFromStr(null, get(status, Codes.INVALID)), status)
    ...
    fun <T> conflict(ex: Throwable, status: Failed.Errored? = null): Result<T, E> =
        Failure(errorFromEx(ex, Codes.CONFLICT), status ?: Codes.CONFLICT)

    fun <T> conflict(err: Err, status: Failed.Errored? = null): Result<T, E> =
        Failure(errorFromErr(err, Codes.CONFLICT), status ?: Codes.CONFLICT)
    ...
    fun <T> errored(msg: String, status: Failed.Errored? = null): Result<T, E> =
        Failure(errorFromStr(msg, get(status, Codes.ERRORED)), status ?: Codes.ERRORED)

    fun <T> errored(ex: Throwable, status: Failed.Errored? = null): Result<T, E> =
        Failure(errorFromEx(ex, get(status, Codes.ERRORED)), status ?: Codes.ERRORED)

    fun <T> errored(err: Err, status: Failed.Errored? = null): Result<T, E> =
        Failure(errorFromErr(err, get(status, Codes.ERRORED)), status ?: Codes.ERRORED)

    fun <T> errored(status: Failed.Errored): Result<T, E> =
        Failure(errorFromStr(null, get(status, Codes.ERRORED)), status)
    ...
    fun <T> unexpected(ex: Throwable, status: Failed.Unknown? = null): Result<T, E> =
        Failure(errorFromEx(ex, Codes.UNEXPECTED), status ?: Codes.UNEXPECTED)

    fun <T> unexpected(err: Err, status: Failed.Unknown? = null): Result<T, E> =
        Failure(errorFromErr(err, Codes.UNEXPECTED), status ?: Codes.UNEXPECTED)

    fun <T> unexpected(status: Failed.Unknown): Result<T, E> =
        Failure(errorFromStr(null, get(status, Codes.UNEXPECTED)), status)
    ...
    fun <T> of(
        ...
        failure: Failed.Errored = Codes.ERRORED
    ): Result<T, E> {
        return if (!condition)
            errored(failure)
        else if (t == null)
