...
interface Builder<out E> {
    ...
    fun <T> denied(ex: Throwable, status: Failed.Denied? = null): Result<T, E> = Failure(errorFromEx(ex, Codes.DENIED), status ?: Codes.DENIED)
    ...
    fun <T> ignored(ex: Throwable, status: Failed.Ignored? = null): Result<T, E> = Failure(errorFromEx(ex, Codes.IGNORED), status ?: Codes.IGNORED)
    ...
    fun <T> invalid(ex: Throwable, status: Failed.Invalid? = null): Result<T, E> = Failure(errorFromEx(ex, Codes.INVALID), status ?: Codes.INVALID)
    ...
    fun <T> conflict(ex: Throwable, status:Failed.Errored? = null): Result<T, E> = Failure(errorFromEx(ex, Codes.CONFLICT), status ?: Codes.CONFLICT)
    ...
    fun <T> errored(ex: Throwable, status: Failed.Errored? = null): Result<T, E> = Failure(errorFromEx(ex, get(status, Codes.ERRORED)), status ?: Codes.ERRORED)
    ...
    fun <T> unexpected(ex: Throwable, status: Failed.Unknown? = null): Result<T, E> = Failure(errorFromEx(ex, Codes.UNEXPECTED), status ?: Codes.UNEXPECTED)
